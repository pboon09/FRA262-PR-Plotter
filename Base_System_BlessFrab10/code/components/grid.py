class Grid():
    ...
    def create_grid(self):
        ...
        # Highlight the line in row
        middle_row = self.row // 2
        self.canvas.create_line(
            (x, y + middle_row * 10),
            (x + self.column * 10, y + middle_row * 10),
            width=2, fill=self.color_highlight
        )
        
        # Highlight the vertical line
        middle_col = self.column // 2
        self.canvas.create_line(
            (x + middle_col * 10, y),
            (x + middle_col * 10, y + self.row * 10),
            width=2, fill=self.color_highlight
        )
    
        # กำหนดระยะห่างที่ต้องการ (พิกเซล)
        spacing = 20

        # คำนวณตำแหน่งกลางของ grid
        mid_col = self.column // 2

        # สำหรับเส้นสั้นในแนวแกน y (เส้นแนวนอนที่อยู่ตรงกลางคอลัมน์)
        # กำหนด offset เริ่มต้น (ปรับค่าตามที่ต้องการ)
        start_y = 80  
        for i in range(start_y, self.row * 10, spacing):
            self.canvas.create_line(
                (x + mid_col * 10 - 5, y + i),
                (x + mid_col * 10 + 5, y + i),
                width=1.5, fill=self.color_highlight
            )

        # สำหรับเส้นสั้นในแนวแกน x (เส้นแนวตั้งที่อยู่ที่ฐานแถว)
        # กำหนด offset เริ่มต้น (ปรับค่าให้เหมาะสม)
        start_x = 20  
        # กำหนดตำแหน่ง y ที่ต้องการให้เส้นแนวตั้งปรากฏ (ปรับค่าได้ตามที่ต้องการ)
        base_y = y + (self.row - 1) * 10 - 230  
        for j in range(start_x, self.column * 10, spacing):
            self.canvas.create_line(
                (x + j, base_y - 5),
                (x + j, base_y + 5),
                width=1.5, fill=self.color_highlight
