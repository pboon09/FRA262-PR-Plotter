...
void Trapezoidal_Evaluated(volatile Trapezoidal_GenStruct *trapGen,
		...
		float32_t target_p, float32_t vmax, float32_t amax) { // Update current time (assuming 1ms intervals)
	...
	if (evaTrapezoidal->t <= trapGen->time_total) {
		...
		if (evaTrapezoidal->t <= trapGen->t1) {
			...
			evaTrapezoidal->setposition = initial_p
					+ 0.5f * accel * evaTrapezoidal->t * evaTrapezoidal->t;
		...
		// Constant velocity phase
			...
			evaTrapezoidal->setposition = p1
					+ v1 * (evaTrapezoidal->t - trapGen->t1);
...
void Dual_Trapezoidal_Generator(volatile Dual_Trapezoidal_GenStruct *dualTrapGen,
		float32_t initial_p1, float32_t target_p1, float32_t vmax1,
		float32_t amax1, float32_t initial_p2, float32_t target_p2,
		float32_t vmax2, float32_t amax2) {

	// Generate individual trajectories to determine which one takes longer
	Trapezoidal_Generator(&dualTrapGen->joint1, initial_p1, target_p1, vmax1,
			amax1);
	Trapezoidal_Generator(&dualTrapGen->joint2, initial_p2, target_p2, vmax2,
			amax2);
	...
	if (dualTrapGen->joint1.time_total >= dualTrapGen->joint2.time_total) {
		...
		if (distance2 > 0.0f) {  // Only recalculate if joint 2 actually moves
			...
			float32_t time_ratio = dualTrapGen->joint1.time_total
					/ dualTrapGen->joint2.time_total;
			...
			Trapezoidal_Generator(&dualTrapGen->joint2, initial_p2, target_p2,
					scaled_vmax2, scaled_amax2);
		}
	} else {
		...
		if (distance1 > 0.0f) {  // Only recalculate if joint 1 actually moves
			...
			float32_t time_ratio = dualTrapGen->joint2.time_total
					/ dualTrapGen->joint1.time_total;
			...
			// Regenerate trajectory with scaled parameters
			Trapezoidal_Generator(&dualTrapGen->joint1, initial_p1, target_p1,
					scaled_vmax1, scaled_amax1);
		}
	}
...
void Dual_Trapezoidal_Evaluated(volatile Dual_Trapezoidal_GenStruct *dualTrapGen,
	...
	dualEvaTrapezoidal->isFinised = (dualEvaTrapezoidal->t
			> dualTrapGen->time_total);
}

// Initialize a dual trajectory evaluation structure
void Dual_Trajectory_Init(
		volatile Dual_Trapezoidal_EvaStruct *dualEvaTrapezoidal) {
